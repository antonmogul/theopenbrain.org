...
state_label` for regular states and `state_label` for highlights. To reproduce the static
`states` array exactly:
- Figures whose static `states` are **descriptive sentences** (Pupillary,
  Phototransduction, TheVisualCycle, ImpairedVision) → put the sentence in
  `state_description` (label may be `Step N`).
- Figures whose static `states` are **short labels** (EyeStructur, SynapticArchitecture,
  Photoreceptors, RetinalCellTypes×3) → put the label in `state_label`,
  `state_description` NULL.
- Highlights (Pupillary/Phototransduction/TheVisualCycle) → `state_label` is the highlight
  term.
...
Wrap all 14 in a single `BEGIN…COMMIT`. Guard each with `IF v_anim_id IS NULL THEN RAISE
NOTICE …` so a missing key is loud, not silent.

**Generation.** The script is generated by a small Node helper
(`scripts/seed/gen-chapter1-anim-states.mjs`) that reads `animations.json` and emits the
SQL, so the split can never drift from source. The committed artifact is the emitted
`.sql` migration; the generator is committed alongside for provenance.
...
- **CODE** fixes touch the shared transformer. #1 only adds a previously-missing flush
  ...
  with fixtures (incl. the no-nesting Ch2 shape) run in `npm test` before merge.
