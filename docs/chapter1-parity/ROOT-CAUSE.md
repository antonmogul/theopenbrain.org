...
`FullScreenIllustration` *and* `IllustrationInline` for the same paragraph.
...
emits `animationAccommodationVergence`. The DB value is arguably *correct*, but the
...
*degraded* DB tree into `localStorage.sections`, so a stale cached copy can outlive a
...
- **#1 (CODE)** — "half the illustrations *and their subsection bodies* are missing." Fix the nesting flush.
...
- ✅ **Accepted — "body prose faithful" overstated.** Corrected: the nesting bug drops subsection *bodies*, not only figures.
