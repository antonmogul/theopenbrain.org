...
describe("VersionsSection", () => {
  ...
  it("renders a row per version", () => {
    const w = mountSection(VersionsSection, {
      versions: [
        { id: "v1", version_number: "1.0", status: "draft", moduleCount: 2 },
      ],
      versionsLoading: false,
      versionsError: null,
      showNewVersionModal: false,
...
- [ ] **Task 7: `MediaSection.vue`** — consumes `useDashboardMedia` refs; keep the media-_picker_ in the view (it bridges to chapters). Emits `@select`, `@delete`, `@filter`.
