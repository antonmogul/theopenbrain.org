...
function reconstructFromRows(model) {
  ...
  if (model.variants.length)
    out.switches = model.variants.sort((a, b) => a.order - b.order).map((v) => v.label);
  return out;
}
...
const lines = [];
lines.push("-- =============================================================================");
lines.push("-- Chapter 1: animation_states / animation_variants / infoText backfill");
...
lines.push("--   src/assets/json_backend/animations.json — DO NOT hand-edit; re-run the");
lines.push("--   generator instead. (DATA-FIX #2/#5 + #6 of docs/chapter1-parity.)");
lines.push("--");
lines.push("-- WHAT IT DOES: backfills the empty animation_states / animation_variants");
lines.push("--   tables and repairs truncated config.infoText for Chapter 1's interactive");
lines.push("--   figures, so useAnimations emits the states / statesHighlight / switches");
lines.push("--   arrays the components expect.");
lines.push("--");
lines.push("-- IDEMPOTENT: per animation_key it resolves the (existing) UUID, DELETEs its");
lines.push("--   child rows, then re-INSERTs from JSON. Re-running is safe. A missing key");
lines.push("--   RAISEs EXCEPTION and rolls back the whole transaction (exact-parity: fail");
lines.push("--   loud, never partial).");
lines.push("--");
lines.push("-- is_highlight_state is derived from JSON array membership (.states=false,");
lines.push("--   .statesHighlight=true) — this CORRECTS the legacy 20260406000000 seed,");
lines.push("--   which mis-marked several figures and produced empty states[].");
lines.push("--");
lines.push("-- RUN with service_role / superuser (bypasses RLS). Review before applying.");
lines.push("-- =============================================================================");
lines.push("");
lines.push("BEGIN;");
lines.push("");
...
for (const m of models) {
  lines.push(`  -- ${m.key}`);
  lines.push(`  SELECT id INTO v_anim_id FROM animations WHERE animation_key = ${q(m.key)};`);
  lines.push(`  IF v_anim_id IS NULL THEN`);
  lines.push(`    RAISE EXCEPTION 'animation_key % not found — aborting seed', ${q(m.key)};`);
  lines.push(`  END IF;`);

  if (m.states.length || m.variants.length) {
    lines.push(`  DELETE FROM animation_states   WHERE animation_id = v_anim_id;`);
    lines.push(`  DELETE FROM animation_variants WHERE animation_id = v_anim_id;`);
  }

  if (m.states.length) {
    lines.push(`  INSERT INTO animation_states (animation_id, state_label, state_description, order_index, is_highlight_state) VALUES`);
  ...
  if (m.variants.length) {
    lines.push(`  INSERT INTO animation_variants (animation_id, variant_label, order_index) VALUES`);
    const vals = m.variants.map((v) => `    (v_anim_id, ${q(v.label)}, ${v.order})`);
    lines.push(vals.join(",\n") + ";");
  }

  if (m.infoText) {
    // DATA-FIX #6: restore full infoText into config JSONB (idempotent overwrite).
    lines.push(`  UPDATE animations SET config = jsonb_set(COALESCE(config, '{}'::jsonb), '{infoText}', to_jsonb(${q(m.infoText)}::text), true) WHERE id = v_anim_id;`);
  }
...
console.log(`Wrote ${path.relative(REPO, OUT_SQL)} (${models.length} animations).`);
