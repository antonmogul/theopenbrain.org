...
const sectionMapping = {
  ...
  "01-from-retina-to-recognition.md": {
    sectionSlug: "from-retina-to-recognition",
    order: 1,
  },
  "02-attention-economy/02-1-selective-attention.md": {
    sectionSlug: "attention-economy",
    subsection: "2.1",
    order: 0,
  },
  "02-attention-economy/02-2-inattentional-blindness.md": {
    sectionSlug: "attention-economy",
    subsection: "2.2",
    order: 1,
  },
  "02-attention-economy/02-3-change-blindness.md": {
    sectionSlug: "attention-economy",
    subsection: "2.3",
    order: 2,
  },
  "02-attention-economy/02-4-attentional-blink.md": {
    sectionSlug: "attention-economy",
    subsection: "2.4",
    order: 3,
  },
...
async function importChapter2() {
  ...
  const files = fs.readdirSync(chapter2Dir, {
    recursive: true,
    withFileTypes: true,
  });
  ...
  for (const file of files) {
    ...
    const sectionSlug = relativePath.includes("00-intro")
      ? "introduction"
      : relativePath.includes("01-from-retina")
        ? "from-retina-to-recognition"
        : relativePath.includes("02-attention")
          ? "attention-economy"
          : relativePath.includes("03-foveal")
            ? "foveal-peripheral-processing"
            : relativePath.includes("04-saccades")
              ? "saccades-fixations-scanning"
              : relativePath.includes("05-gestalt")
                ? "gestalt-principles"
                : relativePath.includes("06-color")
                  ? "color-perception"
                  : relativePath.includes("07-cognitive")
                    ? "cognitive-load-working-memory"
                    : relativePath.includes("08-perceptual")
                      ? "perceptual-biases"
                      : relativePath.includes("09-motion")
                        ? "motion-animation-temporal"
                        : relativePath.includes("10-case")
                          ? "applied-case-studies"
                          : relativePath.includes("11-looking")
                            ? "looking-forward"
                            : relativePath.includes("appendix-glossary")
                              ? "appendix-glossary"
                              : relativePath.includes("appendix-resources")
                                ? "appendix-resources"
                                : null;
    ...
    const isSubsectionHeader =
      blocks[0]?.type === "heading" && blocks[0]?.level >= 2;
    ...
    if (paraError) {
      ...
      console.log(
        `✅ Inserted paragraph ${orderIndex} into section "${section.slug}"`
      );
    }
  }
