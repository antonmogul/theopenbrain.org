...
async function checkParagraphs() {
  ...
  if (paraError) {
    ...
    console.log(
      "   The dashboard uses service_role key, so you can see the data there."
    );
    return;
  }

  console.log(
    `✅ Found ${paragraphs?.length || 0} paragraphs (showing first 5):\n`
  );
  paragraphs?.forEach((p, i) => {
    console.log(`${i + 1}. Paragraph ID: ${p.id}`);
    console.log(`   Section ID: ${p.section_id}`);
    console.log(
      `   Content preview: ${JSON.stringify(p.content).substring(0, 50)}...\n`
    );
  });
