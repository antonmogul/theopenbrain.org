...
// State + fetch extracted to useProfessorAnalytics (#12). Owns analyticsDateRange
// (watched below to refetch on range change).
const {
  analyticsLoading,
  analyticsError,
  analyticsDateRange,
...
const displayName = computed(() => {
  return (
    profile.value?.full_name || user.value?.email?.split("@")[0] || "Professor"
  );
});
  ...
  >
    ...
    <section v-else-if="activeSection === 'courses'" class="section">
      ...
      <BaseCard v-else-if="showCourseEditor" padding="lg">
        <div class="card-head">
          ...
          <Button variant="ghost" size="sm" @click="closeCourseEditor"
            >← Back</Button
          >
        </div>
        <div class="form-stack">
          ...
          <FormField label="Welcome message (shown to students)">
            <textarea
              v-model="courseForm.welcome_message"
              placeholder="Welcome to this course…"
              rows="2"
            ></textarea>
          </FormField>
          ...
          <div class="form-actions">
            <Button variant="ghost" size="sm" @click="closeCourseEditor"
              >Cancel</Button
            >
            <Button variant="solid" size="sm" @click="saveCourse">{{
      ...
      <div v-else class="card-grid">
        <BaseCard v-for="course in courses" :key="course.id" padding="md">
          <div class="card-head">
            <div>
              ...
              <span class="muted-mono"
                >{{ course.course_code }} · {{ course.semester }}</span
              >
            </div>
            <StatusBadge
              :status="course.is_published ? 'published' : 'draft'"
          ...
          <p v-if="course.description" class="muted">
            {{ course.description }}
          </p>
          <div class="meta-row">
          ...
          <div class="btn-row">
            ...
            <Button
              variant="outline"
              size="sm"
              @click="openInviteModal(course.id)"
              >Invite</Button
            >
            <Button variant="danger" size="sm" @click="deleteCourse(course.id)"
              >Delete</Button
            >
          </div>
        </BaseCard>
    ...
    <section v-else-if="activeSection === 'library'" class="section">
      <SectionHeader
        ...
        title="Reusable teaching content"
      >
        <template #actions>
      ...
      <BaseCard v-if="courses.length > 0" padding="md">
        <div class="add-bar">
          ...
          <Button
            ...
            @click="
              addModulesToCourse(document.getElementById('target-course').value)
            "
          >
            Add to course
          </Button>
      ...
      <div v-else class="stack">
        ...
        >
          <div class="module-head" @click="toggleModuleExpand(mod.id)">
            ...
            <svg
              class="chevron"
              :class="{ open: expandedModuleId === mod.id }"
              width="20"
              height="20"
              viewBox="0 0 24 24"
              fill="none"
    ...
    <section v-else-if="activeSection === 'students'" class="section">
      <SectionHeader eyebrow="04 · Students" title="Enrolled students">
        <template #actions>
          <Button variant="solid" size="sm" @click="openInviteModal()"
        ...
        >
          <div class="student-row">
            ...
            <div class="student-meta">
              <span v-if="student.student_year"
                >Year {{ student.student_year }}</span
              >
              <span v-if="student.student_major">{{
      ...
      <BaseModal v-model="showInviteModal" title="Invite students" size="lg">
        <div class="form-stack">
          ...
          <div v-if="inviteForm.method === 'url'" class="method-block">
            <Button variant="outline" size="sm" @click="generateInviteUrl"
              >Generate URL</Button
            >
          ...
          <div v-if="inviteForm.method === 'code'" class="method-block">
            <p class="method-help">
              Access codes are a preview <PreviewTag /> — use the invite URL to
              enroll students.
            </p>
            <Button variant="outline" size="sm" @click="generateAccessCode"
            ...
            <div v-if="generatedAccessCode" class="generated-row">
              ...
              <Button
                variant="ghost"
                size="sm"
                @click="copyToClipboard(generatedAccessCode)"
                >Copy</Button
              >
            </div>
          </div>
          ...
          <div v-if="inviteForm.method === 'email'" class="method-block">
            ...
            <p class="method-help">
              Email invitations aren't available yet
              <PreviewTag variant="soon" /> — use the invite URL for now.
            </p>
            <Button variant="solid" size="sm" disabled>Send invitations</Button>
          </div>
        </div>
        <template #footer>
          <Button variant="ghost" size="sm" @click="closeInviteModal"
            >Close</Button
          >
        </template>
      </BaseModal>
    ...
    <section v-else-if="activeSection === 'assessments'" class="section">
      <SectionHeader eyebrow="05 · Assessments" title="Quizzes & assignments">
        <template #actions>
          <Button
            v-if="!showAssessmentEditor"
            variant="solid"
            size="sm"
            @click="openAssessmentEditor()"
      ...
      <BaseCard v-else-if="showAssessmentEditor" padding="lg">
        <div class="card-head">
          ...
          <Button variant="ghost" size="sm" @click="closeAssessmentEditor"
            >← Back</Button
          >
        </div>
        <div class="form-stack">
          ...
          <div class="grid-3">
            <FormField label="Time limit (min)">
              <input
                v-model.number="assessmentForm.time_limit_minutes"
                type="number"
                min="1"
              />
            </FormField>
            <FormField label="Passing score (%)">
              <input
                v-model.number="assessmentForm.passing_score"
                type="number"
                min="0"
                max="100"
              />
            ...
            <FormField label="Max attempts">
              <input
                v-model.number="assessmentForm.max_attempts"
                type="number"
                min="1"
              />
            </FormField>
          ...
          <div class="form-actions">
            <Button variant="ghost" size="sm" @click="closeAssessmentEditor"
              >Cancel</Button
            >
            <Button variant="solid" size="sm" @click="saveAssessment">{{
          ...
          <div class="card-head">
            ...
            <div class="btn-row">
              <Button
                variant="outline"
                size="sm"
                @click="openAssessmentEditor(assessment)"
                >Edit</Button
              >
              <Button
                variant="danger"
          ...
          <div class="mini-stats">
            ...
            <div class="mini-stat">
              <span class="mini-value">{{
                displayPct(assessment.avgScore)
              }}</span>
    ...
    <section v-else-if="activeSection === 'analytics'" class="section">
      ...
      <div v-else class="card-grid">
        <BaseCard
          v-for="analytics in courseAnalytics"
        ...
        >
          ...
          <div class="mini-stats">
            ...
            <div class="mini-stat">
              <span class="mini-value">{{
                displayPct(analytics.completionRate)
              }}</span>
    ...
    <section v-else-if="activeSection === 'collaboration'" class="section">
      ...
      <div>
        ...
        <BaseCard v-else class="mt-3">
          <ListRow
            v-for="course in mySharedCourses"
            :key="course.id"
            :label="course.title"
          >
