...
<template>
  <section class="section">
    <SectionHeader eyebrow="03 · Versions" title="Content versions">
      <template #actions>
        <Button variant="solid" size="sm" @click="showNewVersionModal = true"
          >New version</Button
        >
      </template>
    </SectionHeader>
      ...
      <BaseCard v-for="version in versions" :key="version.id" padding="md">
        ...
        <div class="meta-row">
          ...
          <span v-if="version.published_at"
            >Published {{ formatDate(version.published_at) }}</span
          >
        </div>
        <p v-if="version.release_notes" class="muted">
    ...
    >
      ...
      <template #footer>
        <Button variant="ghost" size="sm" @click="showNewVersionModal = false"
          >Cancel</Button
        >
