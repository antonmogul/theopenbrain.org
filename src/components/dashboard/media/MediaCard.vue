...
// Support both DB column names: media_type (new schema) and animation_type (legacy)
const mediaType = computed(() => props.media.media_type || props.media.animation_type);
...
const typeLabel = computed(() => {
  ...
  return labels[mediaType.value] || 'Unknown';
});
...
const typeColor = computed(() => {
  const colors = {
    ...
    'gsap': 'bg-orange-100 text-orange-700',
    'css': 'bg-teal-100 text-teal-700',
  };
  return colors[mediaType.value] || 'bg-gray-100 text-gray-700';
});
...
const typeIcon = computed(() => {
  ...
  return icons[mediaType.value] || icons[MEDIA_TYPES.IMAGE];
});
</script>
...
<template>
  ...
  >
    ...
    <div class="p-3">
      ...
      <span
        v-if="media.interaction_type"
        class="inline-block mt-1.5 px-1.5 py-0.5 text-[10px] font-medium rounded bg-gray-100 text-gray-600"
      >
        {{ INTERACTION_TYPE_LABELS[media.interaction_type] || media.interaction_type }}
      </span>
    </div>
  </div>
</template>
